...
namespace TicTacToe_Simulation
{
    public class TicTacToeGameState
    {
        ...
        private BST<TicTacToeGameState> stateTree;
        ...
        public TicTacToeGameState(string stateString, ref BST<TicTacToeGameState> tree)
        {
            int position = 0;
            stateTree = tree;
            int numDigits = GetIntFromString(stateString, ref position, 2);
            stateId = GetIntFromString(stateString, ref position, numDigits);
        ...
        public void setWinProbabilities()
        {
            ...
            if (gameCount < 1000 && !IsOver)
            {
                int count = 0;
                int tempKey;
                BSTNode<TicTacToeGameState> tempNode;
                winProbability1 = 0;
                winProbability2 = 0;
                foreach (Move move in statePossibleMoves)
                {
            ...
            if (!IsOver)
            {
                ...
                if (statePossibleMoves.Length != 1)
                {
                    ...
                    foreach (Move currentMove in statePossibleMoves)
                    {
                        ...
                        BSTNode<TicTacToeGameState> tempNode = stateTree.Find(tempId);
